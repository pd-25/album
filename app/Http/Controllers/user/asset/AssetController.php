...
class AssetController extends Controller
{
    ...
    {
        $userId = auth()->user()->id;
        $Getartist = Asset::with('artist_name','label_name')->where('user_id', $userId)->orderBy('id','DESC')->paginate(20);
    ...
    {
        $data['artists'] = $this->artistInterface->userWiseArtist(auth()->user()->id);
        $data['labels'] = $this->labelInterface->userWiseLabel(auth()->user()->id);
        $data['stores'] = $this->artistInterface->getAllStore();
        $data['otherkeyartist'] = $this->artistInterface->getuserwiseOtherkeyartist(auth()->user()->id); 

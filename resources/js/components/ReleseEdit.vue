                        ...
                        <div class="col-md-12 mx-0">
                            ...
                            <form id="msform">
                                ...
                                <fieldset>
                                    ...
                                    <!-- :disabled="ShowFirstValidation"  v-on:click="ValidateForm();"-->
                                    <button  type="button" name="next" class="next btn btn-info" >Next Step</button>
                                </fieldset>
                                <fieldset>
                                    <div class="row" style="background: #f7fdff; padding: 10px; border-radius: 3px;" v-for="(track, index) in tracks" :key="index">
                                        ...
                                        <div class="col-md-12 mb-2">
                                            ...
                                            <select required v-model="track.language_t" class="custom-select">
                                                <option v-for='(item, index) in alllanguage' :key="index" :value='item'>{{item}}</option>
                                            </select>
                                        </div>
                                        <div class="col-md-6 mb-2">
                                    ...
                                    <!-- v-on:click="ValidateFormSEC()" -->
                                    <!-- :disabled="ShowThirdValidation" v-on:click="ValidateFormSEC()" -->
                                    <button type="button" name="previous" class="previous btn btn-secondary mx-2" > Previous</button>
                                    <button type="button" name="previous" class="next btn btn-info" > Next Step</button>
                                </fieldset>
                                <fieldset>
                                    <div class="row">
                                        <div class="col-12">
                                    ...
                                    <!-- :disabled="ShowSecondValidation" v-on:click="SubmitReleseDetails()" -->
                                    <button type="button" name="previous" class="previous btn btn-secondary mx-2" > Previous </button>
                                    <button  type="button" name="next" class="btn btn-success" v-on:click="SubmitReleseDetails()"> Confirm</button>
                                </fieldset>
                            </form>
                        </div>
                    </div>
            ...
            <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
                ...
                <div class="modal-body">
                    ...
                    <div class="row" style="background: #f7fdff; padding: 10px; border-radius: 3px;" v-for="(track, index) in tracks" :key="index">
                        ...
                        <div class="col-md-12 mb-2">
                            <label class="form-label"><b>Language of lyrics <span class="text-danger">*</span></b></label>
                            <select disabled required v-model="track.language_t"  class="custom-select">
                                <option v-for='(item, index) in alllanguage' :key="index" :value='item'>{{item}}</option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-2">
...
export default {
    props:['artists','labels','stores','users', 'alllanguage', 'genre1', 'alldetails'],
    data() {
        return {
            HiddenAssetId:'',
    ...
    methods: {
        GetAllDetailsById() {
            ...
            this.userId =this.alldetails.user_id ;
            ...
            this.has_spotify_asset =this.alldetails.asset_artisat_details.has_spotify_asset ;
            this.spotify_id_ass =this.alldetails.spotify_id_ass ;
            this.has_applemusic_asset =this.alldetails.asset_artisat_details.has_applemusic_asset ;
            this.apple_id_ass =this.alldetails.asset_artisat_details.apple_id_ass ;
            ...
            this.tracks = this.alldetails.track_details.filter((val) => { 
                return val.track_artist_id = JSON.parse(val.track_artist_id)
            });
            this.alldetails.track_details.forEach((value, index) => {
                // this.audio.push('/storage/'+value.audio);
                this.HiddenTrackId.push(value.id);
                this.PreviewAudio.push('/storage/'+value.audio);
            });
            this.contritibutor_track_artist_name= this.alldetails.track_artisat_details.track_artist_name
            this.contritibutor_role= this.alldetails.track_artisat_details.role
            this.contritibutor_share= this.alldetails.track_artisat_details.share
            this.contritibutor_publishing= this.alldetails.track_artisat_details.publishing
            this.CheckStore= JSON.parse(this.alldetails.track_artisat_details.store)

        },
        SubmitReleseDetails(){
            Confirm.show('Update Confirmation','Do you want to Update?','Yes','No',
            () => {
                if(this.userId  && this.language  && this.release_title &&  this.is_various_artist &&  this.genre_one && this.p_copy && this.c_copy && this.previously_release && this.label_id && this.contritibutor_track_artist_name  &&  this.contritibutor_role  &&  this.contritibutor_share  && this.contritibutor_publishing  
                ...
                )
                {
                    ...
                    axios.post('/admin/asset/update',formData,{
